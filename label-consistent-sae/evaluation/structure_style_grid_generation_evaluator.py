...
class StructureStyleGridGenerationEvaluator(BaseEvaluator):
    ...
    def evaluate(self, model, dataset, nsteps=None):
        ...
        for i, data_i in enumerate(dataset):
            ...
            #sp, gl = model(data_i["real_A"].cuda(), command="encode")  # SOLT : unnecessary code

            for j in range(bs):
                image = data_i["real_A"][j:j+1] # w this syntax it returns a list
        ...
        for style_path in style_paths:
            # style_image = style_images[style_path].cuda() # SOLT
            style_image = style_images[style_path].to("cuda:0")
            gls.append(model(style_image, command="encode")[1])

        sps = []
        structure_paths = list(structure_images.keys())
        for structure_path in structure_paths:
            # structure_image = structure_images[structure_path].cuda() # SOLT
            structure_image = structure_images[structure_path].to("cuda:0")
